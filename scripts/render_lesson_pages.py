...
async def disconnect(sid):
    ...
    for members in socketio_rooms.values():
        ...
        "code_notes": [
            ...
            "Подключение к Socket.IO состоит из двух частей: базовый URL сервера, например <code>http://localhost:7001</code>, и отдельный Socket.IO path <code>/socket.io</code>.",
            "Ошибка <code>WebSocket / 403</code> обычно означает, что вы пытаетесь подключиться как raw WebSocket к корню <code>/</code>. Для этой главы нужно выбрать режим <code>Socket.IO</code>, а не <code>WebSocket</code>.",
...
app = socketio.ASGIApp(sio, other_asgi_app=fastapi_app, socketio_path="socket.io")
        ...
        },
        {
            ...
            "items": [
                "Запустите приложение. Если в терминале написано <code>Uvicorn running on http://127.0.0.1:7001</code>, значит порт для тестера - <code>7001</code>.",
                "Откройте <code>http://localhost:8010/socket-tester</code>.",
                "В переключателе выберите <code>Socket.IO</code>, не <code>WebSocket</code>.",
                "В поле <code>URL</code> напишите <code>http://localhost:7001</code>.",
                "В поле <code>Socket.IO path</code> оставьте <code>/socket.io</code>.",
                "В поле <code>Имя пользователя</code> можно оставить <code>student</code>.",
                "В поле <code>Комната</code> можно оставить <code>python</code>.",
                "Нажмите <code>Подключиться</code>. В логе должны появиться события <code>connect</code>, <code>connected</code>, <code>name_set</code>, <code>joined_room</code>.",
                "Для проверки обычного сообщения выберите событие <code>chat_message</code> и payload <code>{\"room\":\"python\",\"message\":\"Привет из тестера\"}</code>.",
                "Для проверки задачи выберите событие <code>leave_room</code> и payload <code>{\"room\":\"python\"}</code>. В ответе должно прийти событие <code>left_room</code>.",
            ],
        },
        {
            "title": "Почему WebSocket / даёт 403",
            "items": [
                "Socket.IO - это не просто прямой raw WebSocket на <code>/</code>.",
                "Socket.IO клиент сначала делает служебный handshake на пути <code>/socket.io</code>.",
                "Если подключиться как raw WebSocket к <code>ws://localhost:7001/</code>, запрос попадёт в корень <code>/</code>, где WebSocket endpoint-а нет.",
                "Поэтому в терминале появляется <code>WebSocket / 403</code>.",
                "Правильная проверка этой главы: режим <code>Socket.IO</code>, URL <code>http://localhost:7001</code>, path <code>/socket.io</code>.",
            ],
        },
    ],
    "chapter11": [
    ...
    "chapter10": [
        {
            ...
            "items": [
                ...
                "<code>socketio_path=\"socket.io\"</code> означает: Socket.IO handshake живёт на пути <code>/socket.io</code>.",
                "В тестере это значит: URL пишется как <code>http://localhost:7001</code>, а path отдельно как <code>/socket.io</code>.",
        ...
        {
            "title": "Что происходит при Socket.IO connect",
            "items": [
                "Клиентская библиотека Socket.IO подключается к базовому URL, например <code>http://localhost:7001</code>.",
                "Путь <code>/socket.io</code> передаётся отдельно как настройка <code>path</code>.",
                "Socket.IO делает служебный handshake, получает технический <code>sid</code> и только потом начинает обмен событиями.",
                "Сервер вызывает handler <code>connect(sid, environ)</code>.",
                "Код сохраняет <code>socketio_clients[sid] = \"anonymous\"</code>.",
                "Сервер отправляет текущему клиенту событие <code>connected</code>.",
                "Если вместо этого открыть raw WebSocket на <code>ws://localhost:7001/</code>, handler <code>connect</code> не вызовется, потому что это не Socket.IO handshake.",
            ],
        },
        {
        ...
        {
            "title": "Как вручную проверить leave_room",
            "items": [
                "Откройте <code>http://localhost:8010/socket-tester</code>.",
                "Выберите режим <code>Socket.IO</code>.",
                "Если ваш проект запущен на <code>7001</code>, введите URL <code>http://localhost:7001</code>.",
                "В поле <code>Socket.IO path</code> оставьте <code>/socket.io</code>.",
                "Оставьте комнату <code>python</code>.",
                "Нажмите <code>Подключиться</code> и дождитесь <code>joined_room</code>.",
                "Выберите событие <code>leave_room</code>.",
                "В payload оставьте <code>{\"room\":\"python\"}</code>.",
                "Нажмите <code>Отправить</code>. В логе должно появиться событие <code>left_room</code>.",
            ],
        },
    ],
    "chapter11": [
